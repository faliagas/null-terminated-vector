...
#ifndef __NULL_TERMINATED_INT_VECTOR_HPP__
#define __NULL_TERMINATED_INT_VECTOR_HPP__ 1
...
namespace i1
{
  ...
  template<typename _Tp>
    struct _NTIV_element
    {
      _Tp __int_value;
      _NTIV_element() : __int_value(0) {}
      _NTIV_element(_Tp __value) : __int_value(__value) {}
      static constexpr _NTIV_element
      null_terminator() noexcept
      { return _NTIV_element{std::numeric_limits<_Tp>::max()}; }
      bool
      is_null() const noexcept
      { return __int_value == null_terminator().__int_value; }
      _NTIV_element&
      operator=(_Tp __value) noexcept
      { __int_value = __value; return *this; }
      operator _Tp() noexcept { return __int_value; }
      operator const _Tp() const noexcept { return __int_value; }
    };

  template<typename _P,
           typename __enable = void,
           typename _Alloc = std::allocator<_NTIV_element<_P>>,
           typename _EAlloc = std::allocator<_P>>
    class null_terminated_int_vector
    : private null_terminated_vector<_NTIV_element<_P>, _Alloc>
    { null_terminated_int_vector() = delete; };

  /**
   *  @brief Helper class for @c null_terminated_vector with integer elements
   *
  ...
  template<typename _P,
           typename _Alloc,
           typename _EAlloc>
    class null_terminated_int_vector<_P,
              typename std::enable_if<std::is_integral<_P>::value>::type,
              _Alloc, _EAlloc>
    : public null_terminated_vector<_NTIV_element<_P>, _Alloc>
    {
    protected:
      typedef _NTIV_element<_P>                          _Tp;
    ...
    public:
      ...
      typedef _NTIV_element<_P>                          element_type;
      ...
      null_terminated_int_vector() = default;

      /**
       *  @brief  Create %null_terminated_int_vector with no elements
       *  @param  __a  An allocator object.
       */
      explicit
      null_terminated_int_vector(const allocator_type& __a) noexcept
      : _Base(__a) { }

      /**
       *  @brief  Create %null_terminated_int_vector with default
       *          constructed elements
       *  @param  __n  The number of elements to create
       *  @param  __a  An allocator
       *
       *  This constructor fills the %null_terminated_int_vector with
       *  @a __n default constructed elements.
       */
      explicit
      null_terminated_int_vector(size_type __n,
                                 const allocator_type& __a = allocator_type())
      : _Base(__n, __a) { }

      /**
       *  @brief  Create %null_terminated_int_vector with copies of
       *          exemplar element
       *  @param  __n  The number of elements to create
       ...
       */
      null_terminated_int_vector(size_type __n, const value_type& __p,
                                 const allocator_type& __a = allocator_type())
      : _Base(__n, _Tp(__p), __a) { }

      /**
       *  @brief  Copy constructor
       *  @param  __x  A %null_terminated_int_vector of identical element
       *          and allocator types
       *
       *  All the elements of @a __x are copied.
       *
       *  The newly-created %null_terminated_int_vector uses a copy
       *  of @a __x 's allocator object (unless the allocator traits
       *  dictate a different object.)
       */
      null_terminated_int_vector(const null_terminated_int_vector& __x)
      : _Base(__x) { }

      /**
       *  @brief Move constructor
       *
       *  The newly-created %null_terminated_int_vector contains the
       *  exact contents of the moved instance.
       *  The contents of the moved instance are a valid, but unspecified
       *  %null_terminated_int_vector.
       */
      null_terminated_int_vector(null_terminated_int_vector&&) noexcept
        = default;
       ...
       */
      null_terminated_int_vector(null_terminated_int_vector&& __rv,
                                 const allocator_type& __m)
      : null_terminated_int_vector(__rv, __m) { }

      /**
       *  @brief  Builds %null_terminated_int_vector from initializer list
       *  @param  __l  An initializer_list
       *  @param  __a  An allocator
       *
       *  Create a %null_terminated_int_vector consisting of copies of
       *  the elements in the initializer_list @a __l.
       *
       *  This will call the element type's copy constructor N times
       *  (where N is @a __l.size()) and do no memory reallocation.
       */
      null_terminated_int_vector(std::initializer_list<value_type> __l,
                                 const allocator_type& __a = allocator_type())
      : _Base(__a)
      {
      ...
      /**
       *  @brief  Build %null_terminated_int_vector from range
      ...
      template<typename _InputIterator,
        ...
        null_terminated_int_vector(_InputIterator __first,
       ...
       *  @param  __x %null_terminated_int_vector of identical element and
       *          allocator types
       *
       ...
       */
      null_terminated_int_vector&
      operator=(const null_terminated_int_vector& __x)
      {
       ...
       *  @param  __x  %null_terminated_int_vector of identical element
       *          and allocator types
       *
       *  The contents of @a __x are moved into this %null_terminated_int_vector
       *  (without copying, if the allocators permit it).
       *  Afterwards @a __x is a valid, but unspecified %null_terminated_int_vector.
       *
       *  Whether the allocator is moved depends on the allocator traits.
       */
      null_terminated_int_vector&
      operator=(null_terminated_int_vector&& __x)
       ...
       *  @param  __l  An initializer_list
       *
       *  This function fills a %null_terminated_int_vector with copies
       *  of the elements in the initializer list @a __l.
       *
       *  Note that the assignment completely changes the
       *  %null_terminated_int_vector and that the resulting
       *  %null_terminated_int_vector's size is the same as the number
       ...
       *  %vector.
       */
      null_terminated_int_vector(std::vector<value_type, _EAlloc>&& __x) noexcept
      : _Base(allocator_type())
      { *this = std::move(__x); }

#if 0
      template<typename _VAlloc>
        null_terminated_int_vector&
      ...
      null_terminated_int_vector&
      operator=(std::vector<value_type, _EAlloc>&& __x) noexcept
      {
        *static_cast<_Base*>(this) = reinterpret_cast<
            std::vector<_Tp, _Alloc>&&>(std::move(__x));
        return *this;
      ...
      convert_to(std::vector<value_type, _EAlloc>& __x)
      {
      ...
      reference
      front() noexcept
      { return _Base::front().__int_value; }
...
#endif /* __NULL_TERMINATED_INT_VECTOR_HPP__ */
